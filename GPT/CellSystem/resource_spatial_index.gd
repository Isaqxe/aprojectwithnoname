...
## Lightweight spatial hash for nearby resource queries.
## Rebuilt periodically so cells do not scan every resource in the scene.
class_name ResourceSpatialIndex
...
func _ready() -> void:
	add_to_group("ResourceSpatialIndexes")
	rebuild()
...
func _process(delta: float) -> void:
	...
	if _timer <= 0.0:
		rebuild()
		_timer = maxf(rebuild_interval, 0.05)

func rebuild() -> void:
	_buckets.clear()
	for node in get_tree().get_nodes_in_group("WorldResources"):
		if not is_instance_valid(node) or not node is Node2D:
			continue
		var resource: Node2D = node as Node2D
		var key: Vector2i = _cell_key(resource.global_position)
		...
		for x in range(min_key.x, max_key.x + 1):
			...
			for node in _buckets[key] as Array:
				if not is_instance_valid(node) or not node is Node2D:
					continue
				var resource: Node2D = node as Node2D
				if origin.distance_squared_to(resource.global_position) <= radius_squared:
					result.append(node)
