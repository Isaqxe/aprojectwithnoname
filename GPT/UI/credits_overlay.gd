...
## All visible credit text is kept in CREDIT_BLOCKS below so it can be edited
## without touching the rendering, scrolling, or audio logic.
...
func _build_overlay() -> void:
	...
	_credits_container.size = Vector2(get_viewport().get_visible_rect().size.x, 0.0)
...
func _add_credit_block(block: Dictionary) -> void:
	...
	label.custom_minimum_size = Vector2(get_viewport().get_visible_rect().size.x, 48.0)
...
func _start_credits() -> void:
	...
	_credits_container.position = Vector2(0.0, viewport_size.y)
	...
	_scroll_tween.tween_property(_credits_container, "position:y", -total_height, SCROLL_DURATION)
